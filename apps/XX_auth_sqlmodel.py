...
def todos(auth: str, id: int | None = None):
    with Session(engine) as session:
        if id:
            return session.exec(
                select(Todo).where(Todo.name == auth).where(Todo.id == id)
            ).one()
...
async def get(request, auth):
    ...
    card = (Card(Ul(*todos(auth), id="todo-list"), header=add, footer=Div(id=id_curr)),)
...
async def put(todo: Todo, auth):
    ...
    with Session(engine) as session:
        ...
        _todo = session.exec(
            select(Todo).where(Todo.name == auth).where(Todo.id == todo.id)
        ).one()
