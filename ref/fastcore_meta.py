...
class BypassNewMeta(FixSigMeta):
    ...
    def __call__(cls, x=None, *args, **kwargs):
        if hasattr(cls, "_new_meta"):
            ...
            not isinstance(x, getattr(cls, "_bypass_type", object))
            or len(args)
            or len(kwargs)
        ):
...
def _funcs_kwargs(cls, as_method):
        ...
        for k in cls._methods:
            ...
            if arg is not None:
                ...
                if isinstance(arg, MethodType):
                    arg = MethodType(arg.__func__, self)
                setattr(self, k, arg)
    ...
    if hasattr(cls, "__signature__"):
        cls.__signature__ = _rm_self(inspect.signature(cls.__init__))
    return cls
...
def funcs_kwargs(as_method=False):
    ...
    if callable(as_method):
        return _funcs_kwargs(as_method, False)
