...
async def _from_body(req, p):
    ...
    d = _annotations(anno)
    cargs = {
        k: _form_arg(k, v, d) for k, v in form2dict(form).items() if not d or k in d
    }
    return anno(**cargs)
...
async def _find_p(req, arg: str, p: Parameter):
    ...
    if res in (empty, None):
        res = nested_idx(req.scope, "session", arg) or None
    if res in (empty, None):
        res = req.query_params.get(arg, None)
    if res in (empty, None):
        frm = await req.form()
...
def flat_xt(lst):
    ...
    if isinstance(lst, (FT, str)):
        lst = [lst]
    for item in lst:
        if isinstance(item, (list, tuple)) and not isinstance(item, FT):
            result.extend(item)
        else:
            result.append(item)
    return result
...
def _find_wsp(ws, data, hdrs, arg: str, p: Parameter):
    ...
    if res is empty or res is None:
        res = headers.get(snake2hyphens(arg), None)
    if res is empty or res is None:
        res = p.default
    # We can cast str and list[str] to types; otherwise just return what we have
    if not isinstance(res, (list, str)) or anno is empty:
...
async def _send_ws(ws, resp):
    ...
    res = (
        to_xml(resp)
        if isinstance(resp, (list, tuple)) or hasattr(resp, "__ft__")
        else resp
    )
...
class WS_RouteX(WebSocketRoute):
    def __init__(
        ...
        *,
        ...
        super().__init__(
            path,
            _ws_endp(recv, conn, disconn, hdrs, before),
            name=name,
...
def _xt_resp(req, resp):
    ...
    if (
        resp
        and "hx-request" not in req.headers
        and not any(getattr(o, "tag", "") == "html" for o in resp)
    ):
        if not titles:
            titles = [Title("FastHTML page")]
        resp = Html(
            Head(*titles, *flat_xt(req.hdrs)),
            Body(bdy, *flat_xt(req.ftrs), **req.bodykw),
            **req.htmlkw,
        )
...
def _resp(req, resp, cls=empty):
    ...
    if cls is not empty:
        return cls(resp)
    if isinstance(resp, (list, tuple, HttpHeader)) or hasattr(resp, "__ft__"):
        return _xt_resp(req, resp)
...
class RouteX(Route):
    def __init__(
        ...
        **bodykw,
        ...
        (
            ...
            self.htmlkw,
            self.bodykw,
        ) = endpoint, hdrs, ftrs, before, after, htmlkw, bodykw
        super().__init__(
            path,
            self._endp,
            methods=methods,
    ...
    async def _endp(self, req):
        resp = None
        req.injects = []
        req.hdrs, req.ftrs, req.htmlkw, req.bodykw = map(
            deepcopy, (self.hdrs, self.ftrs, self.htmlkw, self.bodykw)
        )
        ...
        if not resp:
            resp = await _wrap_call(self.f, req, self.sig.parameters)
        for a in self.after:
...
class RouterX(Router):
    def __init__(
        ...
        **bodykw,
        ...
        self.hdrs, self.ftrs, self.bodykw, self.htmlkw, self.before, self.after = (
            hdrs,
            ftrs,
            bodykw,
            htmlkw or {},
...
def _wrap_ex(f, hdrs, ftrs, htmlkw, bodykw):
    async def _f(req, exc):
        req.hdrs, req.ftrs, req.htmlkw, req.bodykw = map(
            deepcopy, (hdrs, ftrs, htmlkw, bodykw)
        )
        res = f(req, exc)
...
class FastHTML(Starlette):
    def __init__(
        ...
        **bodykw,
        ...
        htmlkw = htmlkw or {}
        if default_hdrs:
            hdrs = [charset, viewport, htmxscr, surrsrc, scopesrc] + hdrs
        if ws_hdr:
            hdrs.append(htmxwsscr)
        excs = {
            k: _wrap_ex(v, hdrs, ftrs, htmlkw, bodykw)
            for k, v in (exception_handlers or {}).items()
        }
        super().__init__(
            debug, routes, middleware, excs, on_startup, on_shutdown, lifespan=lifespan
        )
        self.router = RouterX(
            routes,
    ...
    def route(self, path: str, methods=None, name=None, include_in_schema=True):
        ...
        def f(func):
            ...
            self.router.add_route(
                path, func, methods=m, name=name, include_in_schema=include_in_schema
            )
            return func
...
reg_re_param(
    "static",
    "ico|gif|jpg|jpeg|webm|css|js|woff|png|svg|mp4|webp|ttf|otf|eot|woff2|txt|html",
)
