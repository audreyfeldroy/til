...
def ft_html(tag: str, *c, id=None, cls=None, title=None, style=None, **kwargs):
    kwargs["id"], kwargs["cls"], kwargs["title"], kwargs["style"] = (
        id,
        cls,
        title,
        style,
...
def find_inputs(e, tags="input", **kw):
    ...
    if isinstance(e, list):
        tag, cs, attr = e
        if e[0] in tags and kw.items() <= e[2].items():
            inputs.append(e)
    for o in cs:
...
def __getattr__(tag):
    ...
    def _f(*c, target_id=None, **kwargs):
        return ft_hx(tag, *c, target_id=target_id, **kwargs)
...
def html2ft(html, attr1st=False):
    ...
    def _parse(elm, lvl=0, indent=4):
        ...
        for key, value in sorted(elm.attrs.items(), key=lambda x: x[0] == "class"):
            ...
            attrs.append(
                f'{key.replace("-", "_")}={value!r}'
                if _re_h2x_attr_key.match(key)
                else f"**{{{key!r}:{value!r}}}"
            )
        spc = " " * lvl * indent
