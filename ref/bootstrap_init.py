...
capsels = "figure.d-table figcaption, " + ", ".join(
    f"figure.d-{k}-table figcaption" for k in bst_sz_d.values()
)
bst_hdrs = (
...
def placehold(x, y):
    return f"https://placehold.co/{x}x{y}"
...
def Navbar(
    ...
    cls="",
    itemcls="",
    expand: SizeT = SizeT.Lg,
    toggle_text="More",
    toggle_left=True,
    container: ContainerT = ContainerT.Fluid,
    placement: PlacementT = PlacementT.Default,
    **kw,
):
    ...
    return Nav(
        ...
        cls=f"navbar navbar-expand-{expand} {placement} {cls}",
        **kw,
    )


def NavText(text):
    return Span(text, cls="navbar-text")
...
def Image(
    ...
    **kw,
    ...
    if retina:
        kw["srcset"] = f"{src} 2x"
    return Figure(
        Img(src=src, alt=alt, cls=f"figure-img img-fluid {cls}", **kw),
...
def BstFooter(copy, img, img_href, cs, cls="", **kw):
    ...
    return Container(
        Footer(
            ...
            Ul(
                *[Li(o, cls="nav-item") for o in cs],
                cls="nav col-md-4 justify-content-end",
            ),
            cls=f"{cls} d-flex flex-wrap justify-content-between align-items-center py-3 my-4 border-top",
            **kw,
        )
    )
